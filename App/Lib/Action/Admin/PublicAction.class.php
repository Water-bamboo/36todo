	...
	public function menu() {
        ...
        if(isset($_SESSION[C('USER_AUTH_KEY')])) {
            ...
            if(isset($_SESSION['menu'.$_SESSION[C('USER_AUTH_KEY')]])) {
                ...
                //读取数据库模块列表生成菜单项
                $node    =   M("Node");
				$id	=	$node->where(' level=1 and name="'.GROUP_NAME.'"')->getField("id");
                ...
                foreach($list as $key=>$module) {
                     if(isset($accessList[strtoupper(GROUP_NAME)][strtoupper($module['name'])]) || !empty($_SESSION['administrator'])) {
                        //设置模块访问权限
                        $module['access'] =   1;
            ...
            if(empty($group)) {
                if(empty($_SESSION['expressMenu'])) {
                    // 读取快捷方式
                    $list   =  M('Express')->where('status=1')->field('title,url')->order('sort')->select();
